...
func StartApplication() {
	...
	for {
		...
		fmt.Println("4.The End")
		fmt.Scanln(&choice)

		switch choice {
		case 1:
			fmt.Println("Enter I/P choice")
			fmt.Println("1.Get from user ")
			fmt.Println("2.Get from File")
			fmt.Scanln(&ipchoice)
			switch ipchoice {
			case 1:
				namearr := GetName()
				h.InsertInHash(namearr)
			case 2:
				fmt.Println("enter file name")
				fmt.Scanln(&filename)
				namearr := ReadFile(filename)
				h.InsertInHash(namearr)
			}
		case 2:
			namearr := GetName()
			h.DeleteFromHash(namearr)
